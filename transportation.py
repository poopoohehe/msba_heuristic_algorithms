...
def trans(dist, dcs, stores_vol):
    ...
    import math
    store_dc_pairs = sorted(dist.keys(), key=lambda x: (round(dist[x],-1), -(dcs[x[0]][1]-running_totals[x[0]][1])), reverse=False)
    j=0
    ...
    while len(st_vol.keys()) > 0 :
        ...
        if store in st_vol.keys():  
            ...
            if new_door_total<=dc_door_capacity and new_driver_total<=dc_driver_capacity and new_cubic_total<=dc_daily_capacity:               
